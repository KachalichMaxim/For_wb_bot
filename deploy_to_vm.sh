...
echo "ssh -o StrictHostKeyChecking=no -i '$SSH_KEY' ${SSH_USER}@${SSH_HOST} 'cd ${PROJECT_DIR} && source venv/bin/activate && python3 telegram_bot.py'"
echo ""
echo "Or to run in background with screen:"
echo "ssh -o StrictHostKeyChecking=no -i '$SSH_KEY' ${SSH_USER}@${SSH_HOST} 'cd ${PROJECT_DIR} && screen -dmS wb_bot bash -c \"source venv/bin/activate && python3 telegram_bot.py\"'"
echo ""
echo "To view logs:"
